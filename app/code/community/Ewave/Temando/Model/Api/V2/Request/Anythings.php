...
class Ewave_Temando_Model_Api_V2_Request_Anythings extends Mage_Core_Model_Abstract {
	...
	parent::_construct();
	$this->_init('temando/api_v2_request_anythings');
	...
	if ($item instanceof Mage_Sales_Model_Quote_Item || $item instanceof Mage_Sales_Model_Order_Item || $item instanceof Mage_Sales_Model_Quote_Address_Item || $item instanceof Ewave_Temando_Model_Box) {
	    ...
	    $this->_anythings[] = Mage::getModel('temando/api_v2_request_anything')
		    ->setItem($item);
	}
	return $this;
    }
	...
	foreach ($this->_anythings as $anything) {
	    /* @var $anything Ewave_Temando_Model_Api_V2_Request_Anything */
	...
	foreach ($this->_anythings as $anything) {
		...
		if ($this->_need_optimize && $request && in_array($request['packaging'], array('Satchel/Bag', 'Parcel'))) {
		    $grams_weight = Mage::helper('temando/v2')->getGramsWeight($request);
		    // locate all items or add new box
		    for ($i = 0; $i < $request['quantity']; $i++) {
			$adding = false;
			$found = false;
			foreach ($output as $k => $v) {
			    $cur_weight = Mage::helper('temando/v2')->getGramsWeight($v);
